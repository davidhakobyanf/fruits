...
const FormMultiSelect: React.FC<FormMultiSelectProps> = ({ id, label, name, options: selectOptions, onChange }) => {
  ...
  const select = (index: number, event: React.MouseEvent) => {
    ...
    // Вызываем onChange с новыми значениями
    if (onChange) {
  ...
  return (
    <FormField label={label} htmlFor={id} className="relative">
      <div>
                ...
                >
                  <div className="flex w-full flex-col">
                    {filteredOptions.map((option, index) => (
                      <div key={index}>
                        <div
                          className={`w-full cursor-pointer border-b border-stroke hover:bg-primary/5 dark:border-form-strokedark ${option.selected ? 'bg-primary/10' : ''}`}
                          onClick={(event) => select(index, event)}
                        >
                          <div className="relative flex w-full items-center p-2 pl-2">
                            <input type="checkbox" checked={option.selected} readOnly className="mr-2" />
